 ...
 *
 * Container-adaptive: it measures its own viewport and fills whatever flex
 * parent it's given, so it serves both the full-height Lyrics tab and the
 * shorter space under the video player. Reads the current track and position
 * from the stores; it takes no props and auto-follows the queue.
 */
export function SyncedLyrics() {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      // Self-pause while hidden. When this instance is mounted but its tab isn't
      // showing (the under-video copy on the Queue/Lyrics tabs), its viewport
      // collapses to zero height — do no work, just keep polling so it resumes
      // the instant it's shown again. Skips the spring math, DOM writes and
      // `setActive`, so a second lyrics view never competes with the visible one.
      if (!viewportRef.current || viewportRef.current.clientHeight === 0) {
        last = now;
