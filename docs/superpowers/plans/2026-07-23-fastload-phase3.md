...
- **Truncation guard survives streaming**: a body that ends short of the server-declared length must classify as Retry (after `Reset` if chunks were published) — never a short `Done`. This is the clipped-track bug the old `fetch_once` explicitly guarded; the streaming path must keep the guarantee.
...
3. Truncation-at-EOF guard test: decode SUCCEEDS (EOF looked clean) but tee saw fewer than declared bytes → must classify Retry, never a short Done. (Pure-fn: `finish_or_retry(bytes_seen, declared) -> bool`— same predicate, test both directions.)
...
- Per attempt (inside the `load_with_retry` closure — its return type stays `LoadAttempt`, but `Ready(samples, meta)` becomes `Ready(()/* published via events */)`. Simplest honest shape: change `LoadAttempt` to `enum { Published, Skip, Retry }` and let the closure OWN event publishing; `load_with_retry`'s job — attempts/backoff/stop — is unchanged):
  ...
  3. On decode `Ok`: if `finish_or_retry(seen, declared)` says complete → flush `resampler.finish()` as a last Chunk, send `Done` → `Published`. Truncated → send `Reset` (if any chunk went out) → `Retry`.
